...
class PriorityQueue:
    ...
    def heapify_down(self, i: int, size: int) -> None:
        """
        Taken from remove_min and adjusted slightly to perform heapify down
        """
        size = size
        cur = i
        while cur < self.get_size():
    ...
    def heapify_down_sort(self, i: int, size: int) -> None:
        """
        Taken from remove_min and adjusted slightly to perform heapify down
        """
        cur = i
        while cur * 2 + 1 < size:  # While there is at least one child
            left = cur * 2 + 1  # Left child index
            right = cur * 2 + 2  # Right child index
            smallest = cur

            # Compare left child with the current smallest
            if left < size and self._arr[left].get_key() < self._arr[smallest].get_key():
                smallest = left

            # Compare right child with the current smallest
            ...
            if smallest != cur:
                ...
                cur = smallest  # Move to the next node to continue heapifying
            else:
                break  # If no swap is needed, exit the loop
    ...
    def ip_build(self, input_list: DynamicArray) -> None:
        ...
        for i in range(size // 2 - 1, -1, -1):
            self.heapify_down(i, self._arr.get_size())
