"""General graph and lattice graph data structures."""
...
class Graph:
    ...
    def gen_labels_1000(self):
        # Generate a list of unique labels: A, B, ..., Z, AA, AB, ..., ZZ
        extended_alphabet = []

        # Single letter labels
        for i in range(26):
            extended_alphabet.append(chr(65 + i))  # 'A' to 'Z'

        # Two-letter combinations
        for i in range(26):
            for j in range(26):
                extended_alphabet.append(f'{chr(65 + i)}{chr(65 + j)}')  # 'AA' to 'ZZ'

        # Ensure we only have 1000 labels (in case the graph expects exactly 1000)
        extended_alphabet = extended_alphabet[:1000]

        # Assign each node a random label from the extended alphabet
        for node in self._nodes:
            node._data = random.choice(extended_alphabet)

    def gen_labels_300k(self):
        # Generate a list of unique labels: A, B, ..., Z, AA, AB, ..., ZZ
        extended_alphabet = []
        ...
        for i in range(26):
            for j in range(26):
                for k in range(26):
                    extended_alphabet.append(f'{chr(65 + i)}{chr(65 + j)}{chr(65 + k)}')  # 'AA' to 'ZZ'

        for i in range(26):
            for j in range(26):
                for k in range(26):
                    for p in range(26):
                        extended_alphabet.append(f'{chr(65 + i)}{chr(65 + j)}{chr(65 + k)}{chr(65 + j)}')  # 'AA' to 'ZZ'
    ...
    def set_nodes(self, cur: int) -> None:
        """
        sets all nodes to positive infinity not including cur
        """
        for node in self._nodes:
            if node.get_id() != cur:
                node._data = float('inf')
            else:
                node._data = 0
